   ...
   # scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.5 )
    ...
    for epoch in range(train_epoch):
       ...
       # scheduler.step()
        ...
        torch.save(net.state_dict(), './try22/model_epoch{}.pth'.format(epoch+1))
    ...
    np.save('try22/train_accs.npy', train_accs)
    train_loss = np.array(train_loss)
    np.save('try22/train_loss.npy',train_loss)
    test_accs = np.array(test_accs)
    np.save('try22/test_accs.npy', test_accs)
